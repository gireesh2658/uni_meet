...
const ConfirmModal = ({ open, title, message, confirmLabel = "Confirm", confirmVariant = "destructive", onConfirm, onCancel, children }: ConfirmModalProps) => {
  if (!open) return null;
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center" onClick={onCancel}>
      ...
      >
        ...
        <div className="mt-6 flex justify-end gap-3">
          ...
          <button
            onClick={onConfirm}
            className={`rounded-lg px-4 py-2 text-sm font-medium text-card ${
              confirmVariant === "destructive" ? "bg-destructive hover:bg-destructive/90" : "bg-success hover:bg-success/90"
            }`}
          >
            {confirmLabel}
          </button>
