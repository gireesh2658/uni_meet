...
const FacultyProfile = () => {
  ...
  const [form, setForm] = useState({
    name: user?.name || "",
    email: user?.email || "",
    employeeId: (user as any)?.employeeId || "EMP001",
    department: user?.department || "Computer Science",
    designation: (user as any)?.designation || "Professor",
    bio: (user as any)?.bio || "",
    officeRoom: (user as any)?.officeRoom || "",
    phone: user?.phone || "",
  });
  ...
  return (
    <div className="page-fade-in space-y-6 max-w-2xl">
      ...
      <form onSubmit={handleSave} className="rounded-xl border border-border bg-card p-6 shadow-sm space-y-4">
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
          ...
          <div>
            ...
            <input value={form.employeeId} readOnly className={`${inputCls} bg-muted`} />
          </div>
          <div>
          ...
          <div>
            ...
            <input value={form.designation} onChange={e => setForm(p => ({ ...p, designation: e.target.value }))} className={inputCls} />
          </div>
          <div>
      ...
      <form onSubmit={handlePwChange} className="rounded-xl border border-border bg-card p-6 shadow-sm space-y-4">
        ...
        <button type="submit" className="rounded-lg bg-primary px-6 py-2.5 text-sm font-semibold text-primary-foreground hover:bg-primary/90 transition-colors">Change Password</button>
      </form>
    </div>
  );
};
