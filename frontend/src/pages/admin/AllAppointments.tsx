...
const AllAppointments = () => {
  ...
  const filtered = appointments.filter(a => {
    return (statusFilter === "all" || a.status === statusFilter) && (deptFilter === "all" || a.department === deptFilter);
  });

  const exportCSV = () => {
    const headers = "Student,Faculty,Department,Date,Time,Status,Purpose\n";
    const rows = filtered.map(a => `"${a.studentName}","${a.facultyName}","${a.department}","${a.date}","${a.startTime}","${a.status}","${a.purpose}"`).join("\n");
            ...
            <tbody>
              {filtered.map(a => (
                <tr key={a.id} className="border-b border-border last:border-0 hover:bg-muted/30">
                  ...
                  <td className="px-4 py-3 text-muted-foreground">{a.facultyName}</td>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center" onClick={() => setDetail(null)}>
          ...
          <div className="relative z-10 mx-4 w-full max-w-lg rounded-xl border border-border bg-card p-6 shadow-lg page-fade-in" onClick={e => e.stopPropagation()}>
            ...
            <div className="mt-4 space-y-3 text-sm">
              <div className="flex justify-between"><span className="text-muted-foreground">Student</span><span className="font-medium text-foreground">{detail.studentName}</span></div>
              <div className="flex justify-between"><span className="text-muted-foreground">Faculty</span><span className="text-foreground">{detail.facultyName}</span></div>
